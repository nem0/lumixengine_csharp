...
/// @return 1 is return if processed finished
subprocess_weak int subprocess_finished(struct subprocess_s* const process);
...
int subprocess_finished(struct subprocess_s* const process) {
	unsigned long exit_code;
	if (!GetExitCodeProcess(process->hProcess, &exit_code)) return 1;
	return exit_code != 0x103;
}
